...
contract PerpVault is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuard {
    ...
    /// @notice Virtual shares/assets offset (ERC4626-style) that makes first-depositor
    ///         share-inflation attacks economically futile, even under forced ETH
    ///         donations (e.g. selfdestruct). Also removes the empty-vault div-by-zero.
    ...
    function deposit() external payable nonReentrant returns (uint256 shares) {
        ...
        // Virtual offset: first deposit (balBefore=0, totalShares=0) mints 1:1; a drained
        // vault recapitalizes without panic; donations cannot inflate the share price enough
        // to steal from later depositors.
        shares = (msg.value * (totalShares + VIRTUAL)) / (balBefore + VIRTUAL);
    ...
    function withdraw(uint256 shares) external nonReentrant returns (uint256 ethOut) {
        ...
        ethOut = (shares * (address(this).balance + VIRTUAL)) / (totalShares + VIRTUAL);
